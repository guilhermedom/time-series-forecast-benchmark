...
ui = fluidPage(
    ...
    fluidRow(
        column(
            ...
            helpText("Note: file must have only one column, without header. Time series frequency must be in months.")
        ),
        column(
            3, align = "center",
            dateRangeInput("dateRangeInputID", "Time series data range:",
        ...
        column(
            3, align = "center",
            numericInput("forecastPeriodID", "How many months do you want to forecast?", 24, min = 1, max = 48),
...
server = function(input, output) {
    ...
    observeEvent(input$forecastButtonID, {
        ...
        timeSeries = ts(data, start = c(startYear, startMonth), end = c(endYear, endMonth), frequency = 12)
        
        forecastPeriod = input$forecastPeriodID
        
        trainData = window(timeSeries, start = c(startYear, startMonth), end = c(endYear - 2, endMonth))
        testData = window(timeSeries, start = c(endYear - 2, startMonth), end = c(endYear, endMonth))
        ...
        modelHw3 = hw(trainData, seasonal = "multiplicative", h = forecastPeriod, damped = T, phi = 0.9)
        ...
        output$forecastPlotID = renderPlot({
            ...
            if ("modelNaive" %in% mList) lines(modelNaive$mean, pch = 22, lty = 6, col = "red", lwd = 4)
            if ("modelMeanf" %in% mList) lines(modelMeanf$mean, pch = 22, lty = 5, col = "blue", lwd = 4)
            if ("modelRwf" %in% mList) lines(modelRwf$mean, pch = 22, lty = 4, col = "green", lwd = 4)
            if ("modelHolt" %in% mList) lines(modelHolt$mean, pch = 22, lty = 6, col = "cyan", lwd = 4)
            if ("modelHw" %in% mList) lines(modelHw$mean, pch = 22, lty = 2, col = "black", lwd = 4)
            if ("modelHw2" %in% mList) lines(modelHw2$mean, pch = 22, lty = 1, col = "purple", lwd = 4)
            if ("modelHw3" %in% mList) lines(modelHw3$mean, pch = 22, lty = 6, col = "orangered3", lwd = 4)
            if ("modelArima" %in% mList) lines(modelArima$mean, pch = 22, lty = 5, col = "gold", lwd = 4)
            if ("modelTslm" %in% mList) lines(modelTslm$mean, pch = 22, lty = 4, col = "chocolate1", lwd = 4)
